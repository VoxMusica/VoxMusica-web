...
import { findAlbumImage, MIME_TYPES } from "#services/images/data-images.service"

import type { FastifyInstance, FastifyPluginAsync, FastifyRequest } from "fastify"


export const albumsImageRoute: FastifyPluginAsync = async (app: FastifyInstance) => {
  app.get('/:id/image', {
    schema: {params: getImageSchema}
  }, async (request: FastifyRequest<{Params: GetImageParams}>, reply) => {
    const { id } = request.params

    const image = await findAlbumImage(id)
...
export default albumsImageRoute
