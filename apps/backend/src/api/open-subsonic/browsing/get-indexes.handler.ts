...
import type { IndexArtist, OpenSubsonicRequest } from "@voxmusica/types"
...
export const getIndexesHandler: FastifyPluginAsync = async (app) => {
  type GetIndexesType = OpenSubsonicRequest & { Querystring: { musicFolderId?: string, ifModifiedSince?: number } }
  app.get<GetIndexesType>('/getIndexes', async (request, reply) => {
    ...
    app.cache.set(GET_INDEXES_CACHE_KEY, result, ms('1h'))
